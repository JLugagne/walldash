...
import type { Dashboard, DisplayMode, Widget } from '../../types'
...
/**
 * WidgetGrid owns one Dashboard's Widget Grid: the cell layout, the edit-mode guides,
...
export const WidgetGrid: React.FC<WidgetGridProps> = ({
  ...
  const selectedWidget = isEditMode ? (dashboard.widgets.find((w) => w.id === selectedWidgetId) ?? null) : null
  ...
  const startDrag = (widget: Widget, mode: 'move' | 'resize', e: React.PointerEvent) => {
    ...
    setDragInfo({
      ...
      startX: e.clientX,
      startY: e.clientY,
      cellWidth: rect.width / dashboard.cols,
      cellHeight: rect.height / dashboard.rows,
      originRects: widgetsToRects(dashboard.widgets),
  ...
  useEffect(() => {
    ...
    const gridSize: GridSize = { cols: dashboard.cols, rows: dashboard.rows }
    ...
    return detach
  }, [dragInfo, dashboard, onPersistLayout, onMessage, setPreview])

  const effectiveRects: Rect[] = dragInfo && previewRects ? previewRects : widgetsToRects(dashboard.widgets)
  ...
  const actionBarAtTop = selectedRect !== undefined && selectedRect.row + selectedRect.rowSpan >= dashboard.rows
  ...
  return (
    <>
      <div
        ...
        style={{
          gridTemplateColumns: `repeat(${dashboard.cols}, minmax(0, 1fr))`,
          gridTemplateRows: `repeat(${dashboard.rows}, minmax(0, 1fr))`,
        }}
        onPointerDown={(e) => {
      ...
      >
        {isEditMode && (
          ...
          >
            {Array.from({ length: dashboard.cols * dashboard.rows }).map((_, i) => (
