...
export function LayerPanel({ level, placements, activeLayer, onSelectLayer, onRefreshLevels, onRefreshPlacements }: LayerPanelProps) {
  ...
  const layers = level.layers && level.layers.length > 0 ? level.layers : [
    { name: 'controls', hide_gauges: false },
    { name: 'sensors', hide_gauges: false },
  ]

  const countForLayer = (layerName: string) => placements.filter((p) => (p.layer || 'controls') === layerName).length

  const updateLevelLayers = async (newLayers: Layer[]) => {
    if (!level) return
    setBusy(true)
    setError(null)
    try {
  ...
  const handleAddLayer = () => {
    ...
    if (!trimmed) return
    if (layers.some((l) => l.name === trimmed)) {
      setError(`Layer "${trimmed}" already exists`)
      return
    }
    updateLevelLayers([...layers, { name: trimmed, hide_gauges: false }])
    setNewName('')
  }

  const handleDeleteLayer = (layerName: string) => {
    if (layerName === 'controls') return
    const updated = layers.filter((l) => l.name !== layerName)
    updateLevelLayers(updated)
  }

  const handleToggleHideGauges = (layerName: string) => {
  ...
  const startRename = (layerName: string) => {
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 overflow-y-auto">
        {layers.map((layer, idx) => {
          ...
          const count = countForLayer(layer.name)
          ...
          return (
            <div
              key={layer.name}
              onClick={() => {
                if (!isEditing) onSelectLayer(layer.name)
              }}
            ...
            >
                ...
                {isEditing ? (
                  <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') handleRenameLayer(layer.name)
                    ...
                    <button
                      type="button"
                      onClick={() => handleRenameLayer(layer.name)}
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  handleToggleHideGauges(layer.name)
                }}
                title={layer.hide_gauges ? 'Show gauges for zones in this layer' : 'Hide gauges for zones in this layer'}
                className={`w-6 h-6 rounded-md shrink-0 transition-colors flex items-center justify-center cursor-pointer ${
                  layer.hide_gauges
                    ? 'bg-amber-900/30 text-amber-400 hover:bg-amber-900/50'
                    : 'hover:bg-slate-800 text-slate-500 hover:text-white'
                }`}
              >
                {layer.hide_gauges ? <SlidersHorizontal className="w-3.5 h-3.5" /> : <Gauge className="w-3.5 h-3.5" />}
              </button>
