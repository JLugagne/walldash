...
export const WidgetFrame: React.FC<WidgetFrameProps> = ({
  ...
  <div
    className={`w-full h-full flex flex-col bg-slate-900/80 border border-slate-800 shadow-xl overflow-hidden ${
      dense ? 'rounded-xl px-2 py-1 gap-0.5' : 'rounded-2xl px-3 py-2 gap-1'
    }`}
  >
    <div className="h-[18px] shrink-0 flex items-center justify-between gap-1.5 min-w-0">
      <div className="flex items-center gap-1 min-w-0">
        ...
        <span className="text-[11px] leading-none font-semibold text-slate-400 truncate" title={label}>
          {label}
        </span>
      </div>
