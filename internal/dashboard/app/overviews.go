...
// ListOverviews retrieves all overview dashboards ordered by order ascending.
func (a *App) ListOverviews(ctx context.Context) ([]domain.OverviewDashboard, error) {
	log := logger.LoggerFromContext(ctx)
	dashboards, err := a.overviewsRepo.FindAllOverviews(ctx)
	if err != nil {
		log.WithError(err).Error("failed to list overview dashboards")
		return nil, err
	}
	return dashboards, nil
}

// GetOverview retrieves an overview dashboard by its identifier.
func (a *App) GetOverview(ctx context.Context, id string) (domain.OverviewDashboard, error) {
	log := logger.LoggerFromContext(ctx)
	dashboard, err := a.overviewsRepo.FindOverviewByID(ctx, id)
	if err != nil {
		log.WithError(err).WithField("overview_id", id).Error("failed to get overview dashboard")
...
// CreateOverview validates and creates a new overview dashboard.
func (a *App) CreateOverview(ctx context.Context, actor domain.Actor, overview domain.OverviewDashboard) (domain.OverviewDashboard, error) {
	log := logger.LoggerFromContext(ctx)
	...
	if strings.TrimSpace(overview.ID) == "" {
	...
	if err == nil && len(existing) > 0 && overview.Order == 0 {
		...
		overview.Order = maxOrder
	}

	created, err := a.overviewsRepo.CreateOverview(ctx, overview)
	if err != nil {
		log.WithError(err).WithField("overview_id", overview.ID).Error("failed to create overview dashboard")
		return domain.OverviewDashboard{}, err
	}

	log.WithField("overview_id", created.ID).WithField("actor", actor.UserID).Info("overview dashboard created successfully")
	return created, nil
}
...
func (a *App) UpdateOverview(ctx context.Context, actor domain.Actor, overview domain.OverviewDashboard) (domain.OverviewDashboard, error) {
	...
	if strings.TrimSpace(overview.ID) == "" {
		return domain.OverviewDashboard{}, errors.Join(domain.ErrInvalidOverview, errors.New("overview id is required"))
	}
	if strings.TrimSpace(overview.Name) == "" {
		return domain.OverviewDashboard{}, errors.Join(domain.ErrInvalidOverview, errors.New("overview name is required"))
	}
	...
	log.WithField("overview_id", updated.ID).WithField("actor", actor.UserID).Info("overview dashboard updated successfully")
	return updated, nil
}

// DeleteOverview removes an overview dashboard and its child widgets.
func (a *App) DeleteOverview(ctx context.Context, actor domain.Actor, id string) error {
	log := logger.LoggerFromContext(ctx)
	...
	log.WithField("overview_id", id).WithField("actor", actor.UserID).Info("overview dashboard deleted successfully")
	return nil
}
...
// DeleteWidget removes a widget from an overview dashboard.
