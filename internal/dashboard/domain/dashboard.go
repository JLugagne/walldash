...
// Grid dimensions applied to a Dashboard created without explicit ones.
...
// Dashboard. Col and Row are the zero-based top-left cell, ColSpan and RowSpan
...
// Dashboard represents a customizable high-level dashboard view.
...
func (o Dashboard) Validate() error {
	if strings.TrimSpace(o.ID) == "" {
		return errors.Join(ErrInvalidDashboard, errors.New("dashboard id cannot be empty"))
	}
	...
	if o.BackgroundBlur < 0 || o.BackgroundBlur > 32 {
		return errors.Join(ErrInvalidDashboard, errors.New("dashboard background blur must be between 0 and 32"))
	}
	if o.BackgroundDim < 0 || o.BackgroundDim > 100 {
		return errors.Join(ErrInvalidDashboard, errors.New("dashboard background dim must be between 0 and 100"))
	}
	...
	for i := range o.Widgets {
		for j := i + 1; j < len(o.Widgets); j++ {
			if o.Widgets[i].Overlaps(o.Widgets[j]) {
				return errors.Join(ErrInvalidDashboard, errors.New("widgets "+o.Widgets[i].ID+" and "+o.Widgets[j].ID+" overlap"))
			}
		}
	}
	return nil
