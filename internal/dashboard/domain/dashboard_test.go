...
func TestDashboardValidate(t *testing.T) {
	...
	dashboardRejections := map[string]domain.Dashboard{
		"empty id": func() domain.Dashboard {
			o := valid()
			o.ID = "  "
			return o
		}(),
		"empty name": func() domain.Dashboard {
			o := valid()
			o.Name = ""
			return o
		}(),
		"zero cols": func() domain.Dashboard {
			o := valid()
			o.Cols = 0
			return o
		}(),
		"negative rows": func() domain.Dashboard {
			o := valid()
			o.Rows = -1
			return o
		}(),
		"two widgets sharing a cell": func() domain.Dashboard {
			o := valid()
			o.Widgets[1].Col = 1
			o.Widgets[1].Row = 1
			return o
	...
	for name, o := range dashboardRejections {
		t.Run(name+" returns ErrInvalidDashboard", func(t *testing.T) {
			err := o.Validate()
			require.Error(t, err)
	...
	widgetRejections := map[string]domain.Dashboard{
		"a widget leaving the grid": func() domain.Dashboard {
			o := valid()
			o.Widgets[2].Row = 7
			return o
		}(),
		"a widget larger than a narrow grid": func() domain.Dashboard {
			o := valid()
			o.Cols = 2
			o.Rows = 2
		...
		"an invalid child widget": func() domain.Dashboard {
			o := valid()
...
func TestDashboardValidateBackground(t *testing.T) {
	valid := func() domain.Dashboard {
		return domain.Dashboard{
			ID:   "ov-1",
			Name: "Home",
	...
	rejections := map[string]domain.Dashboard{}
	o := valid()
