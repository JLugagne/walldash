...
- **Devices** — every signed-in device with its name, role, status and last-seen time. Rename a device with the pencil icon, change its role (**admin** / **device**), or **revoke** it. Only the **owner** can grant or remove the **owner** role.
- **Revoke** removes the device's access shortly after — within about **15 minutes**. To let it back in, approve it again or send a new invitation.
